        ...
        <div class="modal-dialog ">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <center><h4 class="modal-title" id="myModalLabel">Add New Quiz Takers</h4></center>
...
$getexamid=$_GET['examid'];
    ...
    if(!empty($_POST["studentid"])) {
     ...
     include('dbconnect.php'); 
        $query = mysqli_query($conn,"SELECT * FROM quiz where id='$getexamid'");
        while ($result = mysqli_fetch_array($query)) {
        $status= $result['status'];
        }
        if ($status=="CLOSED"){
          $_SESSION["error_remarks"]="QUIZ IS ALREADY CLOSED , CANNOT ADD NEW EXAMINEE";
          $_SESSION["error"]="error";
               header('location:allowquiztakers.php?examid='.$getexamid.'');
               exit();
        }
 

      $check=mysqli_query($conn,"select * from quiztaker where studentid='" . $studentid . "' AND  examid='" . $getexamid . "'");
     $erow=mysqli_fetch_array($check);
      if($erow>0) {
        $_SESSION["error_remarks"]="Cannot be saved, found quiz taker info duplication";
        ...
        $sql = "INSERT INTO quiztaker VALUES (DEFAULT,'$studentid','$studentname','$getexamid','','','','','','$date')";   
